...
def profiles():
    ...
    try:
        p = Path(_directory).rglob("**/projects/*")         # locally installed pkgs, not yet deployed
        return [x.name for x in p if is_valid_package_dir(x)]
    except IOError:
        return []


def is_valid_package_dir(path):
    # type: (Path) -> bool

    rel = path.relative_to(base)
    if 5 < len(rel.parts):
        # print("ignore package {}, placing too deep".format(path))
        return False
